...
export function squash(obj) {
  const objects = Object.keys(obj).flatMap((key) =>
    squashRecursive(obj[key], key),
  );
  const result = {};
...
export function squashRecursive(obj, path) {
  ...
  return Object.keys(obj).flatMap((key) =>
    squashRecursive(obj[key], path + "." + key),
  );
}
...
const CONSTANTS = await import(
  pathToFileURL(path.join(IN_DIR, "constants.mjs")).href
);

for (const lang of LANGS) {
  const object = await import(
    pathToFileURL(path.join(IN_DIR, lang + ".mjs")).href
  );
