...
import { OpState } from "../global/global.ts";
...
async function createWindow(): Promise<void> {
  ...
  ipcMain.handle(
    "pathJoin",
    (_event: IpcMainInvokeEvent, args: Parameters<typeof path.join>) =>
      path.join(...args),
  );
  ipcMain.handle(
    "openExternal",
    (_event: IpcMainInvokeEvent, args: Parameters<typeof shell.openExternal>) =>
      shell.openExternal(...args),
  );

  protocol.handle("img", async (request) => {
    try {
      let filePath = request.url.replace("img://", "");

      // Fix Windows drive letter (img://C/Users -> C:/Users or img://c/Users -> C:/Users)
      ...
      console.log("Protocol handler received URL:", filePath);

      if (await fs.pathExists(filePath)) {
        console.log("File exists, loading:", filePath);
        return net.fetch("file://" + filePath.replace(/\\/g, "/"));
      }

      const parsedUrl = path.parse(filePath);
      console.log("Parsed URL:", parsedUrl);

      if (!(await fs.pathExists(parsedUrl.dir))) {
        console.log("Directory doesn't exist:", parsedUrl.dir);
        return new Response(null, { status: 404 });
      }

      const dirContents = await fs.readdir(parsedUrl.dir);
      const filePat = new RegExp(
        "^" + general.escapeRegex(parsedUrl.base) + "$",
        "i",
      );
      const filtered = dirContents.filter((file: string) => filePat.test(file));

      if (filtered.length > 0) {
        const matchedPath = path.join(parsedUrl.dir, filtered[0]);
        console.log("Case-insensitive match found:", matchedPath);
        return net.fetch("file://" + matchedPath.replace(/\\/g, "/"));
      }

      console.log("No matching file found for:", parsedUrl.base);
      return new Response(null, { status: 404 });
    } catch (error) {
      console.error("Error in img:// protocol handler:", error);
      return new Response(null, { status: 500 });
    }
  });

  general.checkForUpdates();
  ipcMain.handleOnce("handleProcessArgs", () => {
    general.handleURI(
      process.argv.find((arg: string) => arg.startsWith("cmcmm:")),
    );
    ...
    if (
      operations.filter(
        (operation: Operation) =>
          ...
          operation.state == OpState.QUEUED,
      ).length > 0
    ) {
